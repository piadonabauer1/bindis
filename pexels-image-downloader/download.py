...
while True:
    for photo in api.get_entries():
        ...
        if os.path.exists(photo_path):
            print(f"Skipping (already exists): {photo_path}")
            continue
        ...
        if options["-v"]:
            ...
            print(f"Downloading:\t{photo.original}")
            print(f"Photo url:\t{photo.url}")
            print(f"Photographer:\t{photo.photographer}")
            print(f"Progress:\t{photos}/{total_photos}")
        else:
            if photos == 1:
                print(f"\nDownloading: {photos}/{total_photos}")
            else:
                print(f"Downloading: {photos}/{total_photos}")

        try:
            response = requests.get(download_url, timeout=15)
            response.raise_for_status()  # Falls HTTP-Fehler kommt

            with open(photo_path, "wb") as f:
                f.write(response.content)

        except Exception as e:
            print(f"\n❗ Error downloading {photo.url}: {e}")
            if os.path.exists(photo_path):
                os.remove(photo_path)  # Falls angefangene Datei da ist, sauber löschen
            continue  # => weiter mit dem nächsten Foto
