...
"""
For necessary adjustments before training, refer to lines 7-10 and 12-15.
"""

# Comment out the following lines (8-10) when TRAINING Co-DETR
#custom_imports = dict(imports=["bindi_annotation.AcneDetection.CO_DETR.codetr.codetr"], allow_failed_imports=False)
from AcneDetection.CO_DETR.codetr import codetr


# Uncomment the following lines (13-15) when TRAINING Co-DETR + adjust the path (14)
#import sys
#sys.path.insert(0, '/home/coder/bindis/bindi_annotation/AcneDetection/CO_DETR')
#import codetr
...
model = dict(
    ...
    query_head=dict(
        ...
        as_two_stage=True,
        dn_cfg=dict(
            label_noise_scale=0.5, box_noise_scale=1.0, group_cfg=dict(dynamic=True, num_groups=None, num_dn_queries=100)
        ),
        transformer=dict(
    ...
    rpn_head=dict(
        ...
        loss_cls=dict(type="CrossEntropyLoss", use_sigmoid=True, loss_weight=1.0 * num_dec_layer * loss_lambda),
    ...
    roi_head=[
        dict(
            ...
            bbox_head=dict(
                ...
                loss_cls=dict(type="CrossEntropyLoss", use_sigmoid=False, loss_weight=1.0 * num_dec_layer * loss_lambda),
                loss_bbox=dict(type="GIoULoss", loss_weight=10.0 * num_dec_layer * loss_lambda),
            ),
        )
    ],
    bbox_head=[
        dict(
            ...
            feat_channels=256,
            anchor_generator=dict(
                type="AnchorGenerator", ratios=[1.0], octave_base_scale=8, scales_per_octave=1, strides=[4, 8, 16, 32, 64, 128]
            ),
            bbox_coder=dict(type="DeltaXYWHBBoxCoder", target_means=[0.0, 0.0, 0.0, 0.0], target_stds=[0.1, 0.1, 0.2, 0.2]),
            loss_cls=dict(
                type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=1.0 * num_dec_layer * loss_lambda
            ),
            loss_bbox=dict(type="GIoULoss", loss_weight=2.0 * num_dec_layer * loss_lambda),
            loss_centerness=dict(type="CrossEntropyLoss", use_sigmoid=True, loss_weight=1.0 * num_dec_layer * loss_lambda),
        ),
    ],
...
load_pipeline = [
    ...
    dict(type="RandomResize", scale=image_size, ratio_range=(0.1, 2.0), keep_ratio=True),
...
train_dataloader = dict(
    ...
    dataset=dict(pipeline=train_pipeline, dataset=dict(filter_cfg=dict(filter_empty_gt=False), pipeline=load_pipeline)),
)
