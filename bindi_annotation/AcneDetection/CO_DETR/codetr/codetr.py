...
class CoDETR(BaseDetector):
    def __init__(
        ...
        super(CoDETR, self).__init__(data_preprocessor=data_preprocessor, init_cfg=init_cfg)
        ...
        for i in range(len(bbox_head)):
            if bbox_head[i]:
                bbox_head[i].update(
                    train_cfg=train_cfg[i + head_idx + len(self.roi_head)]
                    if (train_cfg and train_cfg[i + head_idx + len(self.roi_head)] is not None)
                    else None
                )
    ...
    def with_bbox(self):
        ...
        return (hasattr(self, "roi_head") and self.roi_head is not None and len(self.roi_head) > 0) or (
            hasattr(self, "bbox_head") and self.bbox_head is not None and len(self.bbox_head) > 0
        )
    ...
    def loss(self, batch_inputs: Tensor, batch_data_samples: SampleList) -> Union[dict, list]:
        ...
        if self.with_rpn:
            ...
            rpn_losses, proposal_list = self.rpn_head.loss_and_predict(x, rpn_data_samples, proposal_cfg=proposal_cfg)
    ...
    def predict_query_head(self, mlvl_feats: Tuple[Tensor], batch_data_samples: SampleList, rescale: bool = True) -> InstanceList:
        return self.query_head.predict(mlvl_feats, batch_data_samples=batch_data_samples, rescale=rescale)

    def predict_roi_head(self, mlvl_feats: Tuple[Tensor], batch_data_samples: SampleList, rescale: bool = True) -> InstanceList:
        ...
        return self.roi_head[self.eval_index].predict(mlvl_feats, rpn_results_list, batch_data_samples, rescale=rescale)

    def predict_bbox_head(self, mlvl_feats: Tuple[Tensor], batch_data_samples: SampleList, rescale: bool = True) -> InstanceList:
        assert self.with_bbox, "Bbox head must be implemented."
        if self.with_query_head:
            batch_img_metas = [data_samples.metainfo for data_samples in batch_data_samples]
            results = self.query_head.forward(mlvl_feats, batch_img_metas)
            mlvl_feats = results[-1]
        return self.bbox_head[self.eval_index].predict(mlvl_feats, batch_data_samples, rescale=rescale)
